...
def create_python_task(gateway, task_name, fork_environment, task_impl, input_files=[], dependent_modules=[], dependencies=[], is_precious_result=False):
...
'keras==3.2.1', 'tensorflow==2.16.1', 'scikit-learn==1.4.2', 'urllib3==2.2.1'], verbosity=True)
    ...
    for input_file in input_files:
        ...
        input_file_path = os.path.dirname(input_file) if "**" in input_file else input_file
        task.addVariable(input_file, input_file_path)
...
def execute_wf(w):
    ...
    for t in sorted(w.tasks, key=lambda t: t.order):
        ...
        task_to_execute = create_python_task(gateway, t.name, fork_env, t.impl_file, t.input_files, t.dependent_modules, dependent_tasks)
        if len(t.params) > 0:
